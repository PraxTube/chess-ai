...
def test_punish_bishop_on_SP():
    ...
    board = chess.GameState(fen_board=  "rn1qkbnr/pp2pppp/2p5/3p4/3P1Bb1/2P1P3/PP3PPP/RN1QKBNR b KQkq - 0 1")
...
def test_bonus_both_bishops():
    ...
    board = chess.GameState(fen_board="rn1qkbnr/pp2pp1p/2p3p1/3p4/3P1BP1/2P1P3/PP3PP1/RN1QKBNR b KQkq - 0 1") 
    ...
    board = chess.GameState(fen_board="rn1qkbnr/pp3p1p/2p3p1/3p4/P2P1pP1/2P1P3/1P3PP1/RN1QKBNR w KQkq - 0 1")
...
def test_punish_for_pawns_not_right_aligned():
    ...
    board = chess.GameState(fen_board="bbnrnkrq/p1p1p1p1/1p1p1p1p/8/8/1P1P1P1P/P1P1P1P1/BBNRNKRQ w - - 0 1")
    ...
    board = chess.GameState(fen_board="nrbkrbnq/pp3p1p/2p1p1p1/3p4/3P4/2P1P1P1/PP3P1P/NRBKRBNQ w - - 0 1")

    assert(test.punish_for_pawns_not_right_aligned(board, True) == -45)
    assert(test.punish_for_pawns_not_right_aligned(board, False) == 45)

def test_pawn_promotion_poss():
    # ein weißer bauer ist in er vorletzten reihe
    board = chess.GameState(fen_board="nqr1krbb/pppP3p/3Pnpp1/1P6/5P2/8/P1P1pKPP/NQRN1RBB b - - 0 1")
    ...
    board = chess.GameState(fen_board="nqr1krbb/p1p1pppp/1p1Pn3/8/3P4/8/PP2PPPP/NQRNKRBB b - - 0 1")

    assert(test.pawn_promotion_poss(board.board, True)== 150)
    assert(test.pawn_promotion_poss(board.board, False)== 0)

    board = chess.GameState(fen_board="rnbqkbnr/p3pppp/8/1p4B1/6P1/N2P4/Ppp1PP1P/R2QKBNR b KQkq - 0 1")
...
def test_bonus_pawns_in_centre():
    board = chess.GameState(fen_board="brnqknrb/pp4pp/5p2/2ppp3/3PP3/2P2P2/PP4PP/RBNQKNBR w - - 0 1")

    assert(test.bonus_pawns_in_centre(board.board, True) == 60)
    assert(test.bonus_pawns_in_centre(board.board, True) == -60)
    
    board = chess.GameState(fen_board="brnqknrb/ppp3pp/5p2/3pp3/4P3/2P5/PP1P1PPP/RBNQKNBR w - - 0 1")
...
def  test_punish_pawns_same_line():
    ...
    board = chess.GameState(fen_board="brnqknrb/2p2ppp/4p3/pP1P4/1P1P4/8/2P2PPP/RBNQKNBR b - - 0 1")

    assert(test.punish_pawns_same_line(board.board, True) == -60)

    assert(test.punish_pawns_same_line(board.board, False) == 0)

    board = chess.GameState(fen_board="brnqknrb/pppp4/8/4p2p/4pp2/6P1/PPPP3P/RBNQKNBR w - - 0 1")
...
def test_bonus_paws_neighbor_lines():
    board = chess.GameState(fen_board="brnqknrb/8/8/7p/P4p2/2P5/8/RBNQKNBR b - - 0 1")
...
def test_bonus_for_Rook_halfopen_lines():
    ...
    board = chess.GameState(fen_board="brnqknrb/8/8/8/1P3pP1/2P5/8/1BNQKNBR b - - 0 1")
...
def test_bonus_Rook_covered_rook():
    board = chess.GameState(fen_board="brnqknrb/8/8/8/1P3pP1/2P5/R6R/1BNQKNB1 b - - 0 1")

    assert(test.bonus_Rook_covered_rook(board.board, True) == 30)